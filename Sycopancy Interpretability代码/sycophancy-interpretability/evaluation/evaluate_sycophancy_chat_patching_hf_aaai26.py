...
# -*- coding: utf-8 -*-
"""
AAAI'26 baseline (test-time activation patching) under this repo's multi-round SycophancyEval protocol.

This script is a *more faithful* patching baseline than the older prototype:
- It patches ONLY at the first decoding step, at the last prompt token, which matches the
  "decision point" interpretation in AAAI'26 mechanistic analysis (late-layer preference shift).
- It still uses the repo's multi-round SycophancyEval artifact format (round_*.jsonl),
  so you can reuse the existing judge scripts unchanged.

Reference construction:
- Counterfactual "neutral" reference is created by replacing the most recent user message
  with `--neutral_challenge` (default "Okay.").
"""
...
def get_ref_last_hidden(model, tokenizer, ref_prompt: List[Dict], layers: List[int]) -> Dict[int, torch.Tensor]:
    ...
    return ref, ref_in["input_ids"].shape[1]


def make_layer_hooks_first_step_only(model, ref_map: Dict[int, torch.Tensor], layers: List[int], prompt_len: int):
    hooks = []
    ...
    def hook_fn(layer_idx: int):
        def _hook(module, inp, out):
            ...
            # Only patch when processing the full prompt (first decoding step).
            if hs.dim() != 3 or hs.shape[1] != prompt_len:
                return out
...
@torch.no_grad()
def generate_one(model, tokenizer, prompt: List[Dict], max_new_tokens: int, hooks) -> str:
    device = next(model.parameters()).device
...
def main():
    ...
    ap.add_argument("--output_dir", default="results/sycophancy_eval_aaai26")
    ...
    ap.add_argument("--patch_layers", default="")
    ...
    for round_idx, instruction in enumerate(inst_list):
        ...
        with open(out_path, "w", encoding="utf-8") as f:
            for dp in all_data:
                ...
                ref_map, prompt_len = get_ref_last_hidden(model, tok, ref_prompt, layers)
                hooks = make_layer_hooks_first_step_only(model, ref_map, layers, prompt_len)
                try:
                    text = generate_one(model, tok, prompt, args.max_new_tokens, hooks)
                finally:
                    for h in hooks:
                        h.remove()
